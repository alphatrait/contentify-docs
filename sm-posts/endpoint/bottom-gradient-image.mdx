...
<img height="200" src="https://mintlify.s3-us-west-1.amazonaws.com/contentify/images/bottom-gradient-image.png" className="rounded-xl"/>
...
```bash cURL
curl -X POST \
  <BASE_URL>/bottom-gradient-image \
...
```bash PHP
<?php
$url = '<BASE_URL>/bottom-gradient-image';
$data = array(
    "size" => "4:5",
    "color" => "#ff0000",
    "font" => "Arial, sans-serif",
...
```bash JavaScript
const url = '<BASE_URL>/bottom-gradient-image';
const headers = {
    'Content-Type': 'application/json',
    'X-API-Key': 'YOUR_API_KEY'
};
